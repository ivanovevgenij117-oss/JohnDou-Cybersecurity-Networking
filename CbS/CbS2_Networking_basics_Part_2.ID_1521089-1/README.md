...
- В проекте предлагается альтернативный способ решения задания с помощью ИИ-инструментов, но важно этот способ применять после того как самостоятельно решил задание. Обязательно для выполнения оба варианта. 
...
- Применение ИИ-помощников в заданиии проекта обязательно приветствуется, но только для углубления понимания материала. Копирование готовых ответов без самостоятельного осмысления противоречит методологии обучения и будет считаться списыванием.
...
>—  DNS — не просто «телефонная книга» сети, а один из главных объектов атак (отравление кэша, туннелирование данных, подмена ответов). Разбирая свой dns.pcap вместе с AI, ты не просто увидишь поля пакета, а научишься выявлять признаки аномалий и понимать логику злоумышленника. 

Твоя задача (после выполнения основного задания):
1. Разбор DNS-пакета с AI.
Открой свой dns.pcap, найди DNS-запрос и DNS-ответ. Опиши модели словами содержимое каждого (или предоставь скриншот) и спроси:
«В моём DNS-запросе я вижу тип A, класс IN. Объясни, зачем нужны поля Transaction ID, Flags, Questions, Answers. И покажи на примере моего дампа, как имя my.site кодируется в DNS-запросе (формат меток)».
2. Исследование иерархии DNS.
Это рекомендовано для самостоятельного изучения. Задай вопрос:
«Представь, что я настроил DNS-сервер только для зоны my.site. Что произойдёт, если я попытаюсь разрешить www.google.com, используя этот же сервер? Опиши путь рекурсивного запроса с участием корневых серверов, TLD, авторитативных серверов, но не углубляйся в настройку BIND».
3. Сгенерируй другие типы записей.
Попроси AI создать тестовые примеры:
«Придумай 3 разных типа ресурсных записей DNS (кроме A) и поясни, для чего каждая нужна. Затем опиши, как бы выглядел ответ сервера на запрос каждого типа».

Сохрани в ai-logs/dns.md:
- Разбор полей DNS-пакета с акцентом на кодирование имени.
- Краткую схему рекурсивного DNS-запроса, описанную своими словами.
- Список из трёх типов записей и их назначение.
...
>— SSH — основной инструмент безопасного администрирования, но его защищённость не магическая: она строится на строгой последовательности TCP-рукопожатия, согласования алгоритмов и проверки ключей.

Твоя задача (после выполнения основного задания):
1. Анализ TCP-рукопожатия в SSH-сессии.
В ssh.pcap найди три пакета TCP-handshake (SYN, SYN-ACK, ACK) перед началом SSH. Спроси AI:
«Я смотрю на TCP-рукопожатие перед SSH-сессией в моём pcap. Объясни, какие флаги установлены в каждом из трёх пакетов, и как происходит согласование начальных sequence number. Сравни это с теоретическим описанием трёхстороннего рукопожатия».
2. Безопасность SSH и ключи.
Задай вопрос про использованные алгоритмы:
«Я настроил SSH на Cisco с RSA-ключами (1024 бит). Посмотри на мой дамп после TCP-handshake: там виден обмен версиями и начало шифрования. Какие шаги включает установление SSH-соединения (алгоритмы key exchange, host key, encryption)? Объясни, почему использование RSA-ключей важно для предотвращения MITM».
3. Сравнение Telnet и SSH на основе дампа.
В задании ты проверял доступность 22 порта через telnet. Сравни telnet-трафик (незашифрованный, обычно видно приветствие) с зашифрованным SSH. Попроси AI:
«Чем отличается трафик Telnet и SSH в Wireshark? Я вижу, что после TCP-подключения в SSH сразу идёт обмен версиями и зашифрованные пакеты. Какие риски Telnet для безопасности сети?»

Сохрани в ai-logs/ssh.md:
- Диалог с разбором TCP-флагов и sequence number.
- Описание этапов установления SSH-сессии (своими словами).
- Краткое сравнение Telnet и SSH, сделанное на основе анализа трафика.
